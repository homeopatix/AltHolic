...
function CreateUIShowSharedStorage()
	...
	local howToDisplay;
	local MessageBtLines;
	local UIShowSearchBtNoLines;
	local buttonClose;
	local centerWindow = {};
	...
	local itemTmp4;
	local sizeBoxHeight;
	local heightSizeOfMap = 0;
	local tmpItemsCount = 0;
	local datasFromSearch = {};
	local nbrIcones = 0;
	local posx = 10;
	local posy = 5;
	...
	-- Sort slot keys directly so duplicate item names remain distinct.
	local newtable = AltHolicUtil.SortedItemSlots(SharedStorageVault);
